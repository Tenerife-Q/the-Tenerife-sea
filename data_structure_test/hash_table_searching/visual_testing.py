...
class HashTableVisualizer:
    def __init__(self, root):
        self.root = root
        self.root.title("哈希表可视化工具 - 三种冲突解决方法演示")
    ...
    def create_widgets(self):
        ...
        methods = [
            ...
            ("二次探测", "quadratic", "探测序列: h(k), h(k)+1², h(k)+2², ..."),
            ("链地址法", "chain", "相同哈希值形成链表")
        ]
...
"""
    ...
    def open_probing_insert(self, value):
        ...
        for i in range(self.table_size):
            if self.probing_method == 'linear':
                ...
                if i > 0:
                    probe_calc = f"h({value}) + {i} = {index} + {i} = {current_index}"
            else:  # quadratic
                current_index = (index + i * i) % self.table_size
                if i > 0:
                    probe_calc = f"h({value}) + {i}² = {index} + {i * i} = {current_index}"
        ...
        return False, steps, probe_sequence
    ...
    def insert(self):
        ...
        if self.probing_method in ["linear", "quadratic"]:
            ...
            if success:
                self.status_var.set(f"✅ 插入成功！值: {value}, 探测步数: {steps}, 探测序列: {probe_sequence}")
            else:
    ...
    def search(self):
        ...
        if self.probing_method in ["linear", "quadratic"]:
            ...
            if found:
                self.status_var.set(f"🔍 查找成功！值: {value}, 位置: {index}, 探测步数: {steps}")
                self.highlighted_cells = probe_sequence
            else:
                self.status_var.set(f"❌ 查找失败：值 {value} 不存在！探测了{steps}次")
                self.highlighted_cells = probe_sequence
        else:
            hash_calc = f"h({value}) = {value} % {self.table_size} = {self.hash_function(value)}"
            found, index, pos = self.chain_search(value)
            if found:
                self.status_var.set(f"🔍 查找成功！值: {value}, 哈希位置: {index}, 链表位置: {pos}")
                self.highlighted_cells = [index]
    ...
    def draw_chaining(self, width, height):
        ...
        for i in range(self.table_size):
            ...
            if chain_length == 0:
                ...
                # 获取排序后的链表（从小到大）
                sorted_chain = sorted(self.chain_table[i])

                # 绘制链表节点（竖向排列，从小到大）
                for j, value in enumerate(sorted_chain):
                    # 计算节点位置
                    ...
                    if cell_width > 80:
                        ...
                        if hash_val == i:
                            self.canvas.create_text(x + cell_width + 20, current_y + cell_height // 2,
                                                    text=f"h({value})={hash_val}",
                                                    font=("Arial", min(8, cell_width // 12)), fill="darkgreen")

                    # 绘制指向下一个节点的箭头（除最后一个节点）
                    if j < len(sorted_chain) - 1:
                        next_y = current_y + cell_height + 8
                        if next_y + cell_height <= height - 60:  # 确保下一个节点不超出边界
                            self.canvas.create_line(x + cell_width // 2, current_y + cell_height,
                                                    x + cell_width // 2, next_y,
                                                    arrow=tk.LAST, width=2, fill="green")
                    else:
                        # 最后一个节点指向NULL
                        null_y = current_y + cell_height + 3
                        if null_y + 20 <= height - 40:
                            self.canvas.create_line(x + cell_width // 2, current_y + cell_height,
                                                    x + cell_width // 2, null_y + 12, width=2, fill="gray")
                            self.canvas.create_text(x + cell_width // 2, null_y + 20,
                                                    text="NULL", font=("Arial", min(8, cell_width // 12)), fill="gray")
                ...
                # 显示排序状态
                if chain_length > 1:
                    self.canvas.create_text(x + cell_width // 2, y - 25,
                                            text="有序",
                                            font=("Arial", min(7, cell_width // 12)), fill="purple")
        ...
        # 绘制排序说明
        sort_text = "链表按从小到大排序"
        self.canvas.create_text(width // 2, 85, text=sort_text,
        ...
        if max_chain_length > 0:
            ...
            if max_chain_length > max_visible_nodes:
                self.canvas.create_text(width // 2, height - 15,
                                        text=f"⚠️ 链表过长，仅显示前{max_visible_nodes}个节点",
                                        font=("Arial", 10), fill="red")

    def draw_probe_arrows(self, x_start, y_center, cell_width, cell_height):
        """绘制探测序列箭头（自适应）"""
        y_arrow = y_center + cell_height // 2 + 25

        for i in range(len(self.highlighted_cells) - 1):
            current_pos = self.highlighted_cells[i]
            next_pos = self.highlighted_cells[i + 1]

            x1 = x_start + current_pos * (cell_width + 10) + cell_width // 2
            x2 = x_start + next_pos * (cell_width + 10) + cell_width // 2

            # 绘制箭头
            self.canvas.create_line(x1, y_arrow, x2, y_arrow,
                                    arrow=tk.LAST, width=2, fill="red")

            # 标注步骤（只在有足够空间时显示）
            if abs(x2 - x1) > 40:  # 确保有足够空间显示文字
                mid_x = (x1 + x2) // 2
                self.canvas.create_text(mid_x, y_arrow - 12,
                                        text=f"步骤{i + 1}", font=("Arial", min(8, cell_width // 12)), fill="red")
