...
int main() {
    ...
    printf("插入：");
    for (int i = 0; i < n; i++) {
        chainInsert(table, keys[i], midSquare);
        printf("%d ", keys[i]);
    }
    printf("\n");
    printTable(table);
    ...
    for (int i = 0; i < 3; i++) {
        int asl = chainSearch(table, searchKeys[i], midSquare);
        if (asl)
    ...
    int total = 0, found = 0;
    for (int i = 0; i < n; i++) {
        int asl = chainSearch(table, keys[i], midSquare);
