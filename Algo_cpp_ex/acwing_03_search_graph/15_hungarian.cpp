 ...
 *   
 *   【代码模拟演示 (以男生 S1, S2 和 女生 D1, D2 为例)】
 *   关系：S1 喜欢 D1, D2； S2 只喜欢 D1。
 *   
 *   --- 第一轮：帮 S1 找对象 (i=1) ---
 *   1. 清空 st 数组 (st={false, false})。
 *   2. 调用 find(1)。
 *   3. S1 第一个看中的是 D1。
 *      - D1 还没被预定 (!st[1]) -> 标记 st[1]=true。
 *      - D1 没对象 (match[1]==0)。
 *      - 【匹配成功】：match[1] = 1 (D1 的男友是 S1)。
 *      - return true。
 *   4. res ++ (目前撮合 1 对)。
 *   
 *   --- 第二轮：帮 S2 找对象 (i=2) ---
 *   1. 清空 st 数组 (st={false, false})。注意！每一轮都要清空！
 *   2. 调用 find(2)。
 *   3. S2 第一个看中的是 D1。
 *      - D1 还没被预定 (!st[1]) -> 标记 st[1]=true。
 *      - D1 有对象了！是 1号男生 (match[1]==1)。
 *      - 【冲突解决方案】：让 1号男生去找别的备胎！
 *      - 递归调用 find(1)。
 *      
 *      --- 进入递归 find(1) ---
 *      1. S1重新看他的名单。
 *      2. D1? 
 *         - 此时 st[1] 已经是 true 了 (S2 正在盯着 D1 呢)。
 *         - 跳过 D1。
 *      3. S1 第二个看中的是 D2。
 *         - D2 没被预定 (!st[2]) -> 标记 st[2]=true。
 *         - D2 没对象 (match[2]==0)。
 *         - 【S1 移情别恋成功】：match[2] = 1 (D2 的男友变成 S1)。
 *         - return true。
 *      ------------------------
 *      
 *      - 既然 S1 成功找到了 D2，那 D1 就空出来了。
 *      - 【S2 上位成功】：match[1] = 2 (D1 的男友变成 S2)。
 *      - return true。
 *   4. res ++ (目前撮合 2 对)。
 *   
 *   最终结果：2 对。
 ...
 *  2 2 4
    ...
    2 2
    输出：2
 *    解释：S1喜欢D1和D2，S2喜欢D1和D2。最终S1和D2，S2和D1在一起了。
...
bool find(int u) {
    for(int i = h[u]; i != -1; i = ne[i]) {
        ...
        这个限制条件非常关键！它的作用是：
        在这一次帮 u 找对象的过程中，j 这个女生已经被预定过了，别老盯着这同一个女生看。
        也就 find(match[j]) 以防让原配找备胎找到这个女生，导致死循环。
        if(!st[j]) { 
            st[j] = true;
            if(match[j] == 0 || find(match[j])) {
                match[j] = u;
