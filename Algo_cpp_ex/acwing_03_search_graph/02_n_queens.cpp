 ...
 * 
 *      (回到 dfs(0))
 *      - 刚才 (0,0) 的路走不通! (N=4 时，第一个皇后放 (0,0) 确实无解)
 *      - [BACKTRACK]: 撤销 (0,0). g[0][0]='.'. col[0]=dg[0]=udg[4]=false.
 *      - 尝试 i=1. 安全! (col[1], dg[1], udg[4-0+1=5] 都没被占)
 *      - [ACTION]: 在 (0,1) 放 Q. 开启雷达: col[1], dg[1], udg[5].
 *      - 棋盘:
 *        . Q . .
 *        . . . .
 *        . . . .
 ...
 *      2. 【dfs(1) 开始】: u=1 (第1行)
 *         - i=0. 安全!
 *           检查: col[0]没占. dg[1+0=1]...等等! dg[1]被(0,1)占了!
 *           (0,1)的dg索引=0+1=1. (1,0)的dg索引=1+0=1. 冲突!
 *           -> (0,1) 和 (1,0) 在同一条副对角线(/)上. 跳过.
 *         - i=1. col[1] 冲突.
 *         - i=2. 安全?
 *           col[2] ok. dg[1+2=3] ok. udg[4-1+2=5]...冲突!
 *           (0,1)的udg索引=4-0+1=5. (1,2)的udg索引=4-1+2=5. 
 *           -> (0,1) 和 (1,2) 在同一条主对角线(\)上. 跳过.
 *         - i=3. 安全!
 *           col[3] ok. dg[1+3=4] ok. udg[4-1+3=6] ok.
 *         - [ACTION]: 在 (1,3) 放 Q. 开启雷达: col[3], dg[4], udg[6].
 *         - 棋盘:
 *           . Q . .
 *           . . . Q
 *           . . . .
 ...
 *         3. 【dfs(2) 开始】: u=2 (第2行)
 *            - i=0. 安全!
 *              col[0] ok. dg[2+0=2] ok. udg[4-2+0=2] ok.
 *            - [ACTION]: 在 (2,0) 放 Q. 开启雷达: col[0], dg[2], udg[2].
 *            - 棋盘:
 *              . Q . .
 *              . . . Q
 *              Q . . .
 ...
 *               - i=1. col[1] 冲突.
 *               - i=2. 安全!
 *                 col[2] ok. dg[3+2=5] (之前是(0,1)占了udg[5],不是dg[5]). ok.
 *                 udg[4-3+2=3] ok.
 *               - [ACTION]: 在 (3,2) 放 Q.
 *               - 棋盘:
 *                 . Q . .
 ...
 * ============================================
 * 
 *   【深度解密：为什么是对角线公式是 u+i 和 n-u+i ？】
 * 
 *   我们需要用一维数组来唯一标识每一条斜线。
 *   
 *   1. 副对角线 (diagonal /) -> dg[u+i]
 *      - 在直角坐标系中，这种斜线的方程是 y = -x + b。
 *      - 移项得：b = x + y。
 *      - 在矩阵中，x是行(u)，y是列(i)。
 *      - 所以每一条副对角线上的点，它们的 (行号 + 列号) 都是常数。
 *      - 例如 (0,1), (1,0) 都在 b=1 的线上。
 *      - 所以用 `u + i` 作为数组下标，就能索引到这条线。
 * 
 *   2. 主对角线 (anti-diagonal \) -> udg[n-u+i]
 *      - 这种斜线的方程是 y = x + b。
 *      - 移项得：b = y - x。
 *      - 也就是 (列号 - 行号) 是常数。
 *      - 问题是 i - u 可能是负数（比如行2列0，0-2=-2）。
 *      - 数组下标不能是负数，所以我们要加上一个偏移量 n。
 *      - 公式变成 `n + i - u` (即 `n - u + i`)。
 *      - 这样最小也是 1 (当i=0, u=n-1)，最大是 2n-1，都在合法范围内。
 *   就是最后差值相等即可，但是数组不能有负数，所以加上 n 作为偏移。
 *   本身那个索引不是特别重要，重要的是它能唯一标识每条斜线，并且不越界。
 * 
 *   【总结】
 *   - dg[sum] 监控的是所有 `行+列 == sum` 的格子。
 *   - udg[diff] 监控的是所有 `列-行+n == diff` 的格子。
 * ============================================
 */
