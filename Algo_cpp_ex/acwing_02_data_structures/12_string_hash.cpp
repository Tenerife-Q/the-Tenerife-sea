...
int main() {
    ...
    // 【预处理：O(n)】
    // 我们需要预处理出所有前缀的哈希值 h[i] 和 P 的次方 p[i]。
    p[0] = 1; // P^0 = 1
    for (int i = 1; i <= n; i ++ ) {
        // 1. 预处理次方 P^i
        // 就像十进制里算出 10, 100, 1000... 方便后面位权对齐。
        p[i] = p[i - 1] * P;
        
        // 2. 预处理前缀哈希
        // 公式：h[i] = h[i-1] * P + str[i]
        // 物理意义【传送带模型】：
        // 想象一个传送带一直在往左滚。每滚一下(*P)，最右边就腾出一个空位。
        // 然后把新来的字符 str[i] 塞进这个最右边的空位(+str[i])。
        // 例: "AB" -> h["AB"] * P + 'C' -> "ABC"
        h[i] = h[i - 1] * P + str[i];
    }
 ...
 * 
 * 2. 处理 '2':
 *    为了把 2 接到 1 后面，1 必须左移一位 (*10)。
 *    当前值 = 1 * 10 + 2 = 12
 * 
 * 3. 处理 '3':
 *    为了把 3 接到 12 后面，12 必须左移一位 (*10)。
 *    当前值 = 12 * 10 + 3 = 123
 * 
 * [P=131]
 * 字符串 "ABC" -> 整数 Val
